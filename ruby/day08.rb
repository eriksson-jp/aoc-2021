...
input = """febca cfagb ecbafd efdcbg cbegdfa fg bgafec gfae acgdb gfc | cgf facdeb ecgfdb afcbge
...
ga befgac fcgedba fabced agfce edgfc gbea aebcf acg cagfdb | gecdf afebcd bgdfac cga""".split("\n")

def taskOne(input)
  output = []
  input.each { |line| 
    line.split("|")[1].split(" ").each { |word| output << word }
  }
  output.select { |word| [2,3,4,7].include?(word.length) }.size
end

def getNumberFromLine(line)
  patterns = line.split("|")[0].split(" ").map { |pattern| pattern.chars.to_set }
  patternMap = {}
  
  patternMap[1] = patterns.detect { |p| p.size == 2 }
  patternMap[4] = patterns.detect { |p| p.size == 4 }
  patternMap[7] = patterns.detect { |p| p.size == 3 }
  patternMap[8] = patterns.detect { |p| p.size == 7 }
  
  patternMap[3] = patterns.filter { |p| p.size == 5 and p.superset?(patternMap[1]) }.first
  patternMap[9] = patterns.filter { |p| p.size == 6 and p.superset?(patternMap[3]) }.first
  patternMap[0] = patterns.filter { |p| p.size == 6 and p.superset?(patternMap[1]) and p.superset?(patternMap[7]) and p != patternMap[9] }.first
  patternMap[6] = patterns.filter { |p| p.size == 6 and p != patternMap[0] and p != patternMap[9] }.first
  patternMap[5] = patterns.filter { |p| p.size == 5 and patternMap[6].superset?(p) }.first
  patternMap[2] = patterns.filter { |p| p.size == 5 and p != patternMap[3] and p != patternMap[5] }.first
  
  pattern_to_number = patternMap.invert
  
  number_patterns = line.split("|")[1].split(" ").map { |pattern| pattern.chars.to_set }
  number = ""
  number_patterns.each { |np|
    number << pattern_to_number[np].to_s
  }
  number.to_i
end
...
def taskTwo(input)
  sum = 0
  input.each { |line| sum += getNumberFromLine(line)}
  sum
end

p "DAY 8.1: #{taskOne(input)}"
p "DAY 8.2: #{taskTwo(input)}"